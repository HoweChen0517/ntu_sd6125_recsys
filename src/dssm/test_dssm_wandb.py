...
os.environ["WANDB_NOTEBOOK_NAME"] = 'f:\\NTU Learn\\DATA MINING\\DMproject\\src\\dssm_wandb.py'
...
wandb.init(
    project="dssm-model_simple_params", 
    config={
        "batch_size": 1024,
        "epochs": 64,
        "learning_rate": 0.001,
...
if __name__ == '__main__':
    ...
    # 保存模型到本地
    model_path = os.path.join(ROOT_PATH, "output", data_set ,'model', "dssm_model_simple_params.pth")
    torch.save(model.state_dict(), model_path)

    # 创建并保存 wandb Artifact
    artifact = wandb.Artifact('dssm-model_simple_params', type='model')
    ...
    # %%
    # 8. model testing
    pred_ts = model.predict(test_model_input, batch_size=config.batch_size)
    test_logloss = log_loss(test[target].values, pred_ts)
    test_auc = roc_auc_score(test[target].values, pred_ts)
